#!/usr/bin/env bash
# test-trend.sh — Test the `cloudrift trend` command.
#
# Runs `cloudrift trend` against the current account and saves the output to
# ./reports/cloudrift-trend-TIMESTAMP.json. Cost Explorer is a global AWS
# service (not region-scoped), so unlike `analyze` there is no --regions flag.
#
# Usage:
#   bash scripts/test-trend.sh
#   bash scripts/test-trend.sh --months 3
#   bash scripts/test-trend.sh --services ec2 s3
#
set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJECT_DIR="$(cd "$SCRIPT_DIR/.." && pwd)"
CLOUDRIFT_PATH="${CLOUDRIFT_PATH:-$PROJECT_DIR/../../cloudrift}"
CLOUDRIFT_MAIN="$CLOUDRIFT_PATH/apps/cli/dist/main.js"
TIMESTAMP=$(date +%Y%m%d_%H%M%S)
REPORTS_DIR="$PROJECT_DIR/reports"
mkdir -p "$REPORTS_DIR"

REPORT_FILE="$REPORTS_DIR/cloudrift-trend-$TIMESTAMP.json"
LOG_FILE="$REPORTS_DIR/cloudrift-trend-$TIMESTAMP.log"

# --- Colors ---
RED='\033[0;31m'
GREEN='\033[0;32m'
YELLOW='\033[1;33m'
CYAN='\033[0;36m'
NC='\033[0m'

log()  { echo -e "${CYAN}[$(date +%H:%M:%S)]${NC} $*"; }
ok()   { echo -e "${GREEN}  ✔${NC} $*"; }
warn() { echo -e "${YELLOW}  ⚠${NC} $*"; }
fail() { echo -e "${RED}  ✗${NC} $*"; }

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  cloudrift trend — Test"
echo "  Started: $(date)"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo ""

# ═══════════════════════════════════════════════════════════════════════════
# PREFLIGHT
# ═══════════════════════════════════════════════════════════════════════════
if [ ! -f "$CLOUDRIFT_MAIN" ]; then
  fail "cloudrift CLI not found at: $CLOUDRIFT_MAIN"
  echo "  Set CLOUDRIFT_PATH or build: cd $CLOUDRIFT_PATH && pnpm nx build cli"
  exit 1
fi
ok "cloudrift CLI found: $CLOUDRIFT_MAIN"
echo ""

# ═══════════════════════════════════════════════════════════════════════════
# RUN: cloudrift trend
# ═══════════════════════════════════════════════════════════════════════════
# Cost Explorer bills $0.01 per API request — this is a real (tiny) charge,
# not a free describe/list call like every other cloudrift scanner.
log "Running: cloudrift trend --format json $*"
echo ""

# stdout (the JSON report) and stderr (warnings, e.g. failed STS account-id
# resolution) are kept separate on purpose: merging them with 2>&1 would
# corrupt the JSON file the moment any warning is printed.
set +e
node "$CLOUDRIFT_MAIN" trend --format json "$@" > "$REPORT_FILE" 2> "$LOG_FILE"
EXIT_CODE=$?
set -e

if [ -s "$LOG_FILE" ]; then
  warn "stderr output (see $LOG_FILE):"
  sed 's/^/    /' "$LOG_FILE"
fi

if [ ! -s "$REPORT_FILE" ]; then
  fail "cloudrift trend produced no output (exit code $EXIT_CODE)."
  echo "  Check credentials and that Cost Explorer is enabled for this account."
  exit 1
fi

ok "Report saved: $REPORT_FILE"
if [ "$EXIT_CODE" -ne 0 ]; then
  warn "Exit code $EXIT_CODE — command reported an error, see log above."
fi
echo ""

# ═══════════════════════════════════════════════════════════════════════════
# QUICK SUMMARY
# ═══════════════════════════════════════════════════════════════════════════
log "Quick summary:"
echo ""

python3 -c "
import json, sys

try:
    data = json.load(open('$REPORT_FILE'))
except Exception as e:
    print(f'  Could not parse JSON: {e}')
    sys.exit(0)

months = data.get('months', [])
filtered = data.get('filteredServices')

print(f'  Account: {data.get(\"meta\", {}).get(\"accountId\", \"?\")}')
print(f'  Scope: {\", \".join(filtered) if filtered else \"all services\"}')
print(f'  Months covered: {len(months)}')
for m in months:
    label = m.get('month', '?')
    cost = m.get('totalUsd', 0)
    final = m.get('final', True)
    suffix = '' if final else '  (estimated — current, still-open month)'
    print(f'    - {label}: \${cost:.2f}{suffix}')

if len(months) >= 2:
    first, last = months[0]['totalUsd'], months[-1]['totalUsd']
    if first != 0:
        pct = (last - first) / first * 100
        print(f'  First -> last month: {pct:+.1f}%')
    else:
        print(f'  First -> last month: n/a (first month was \$0.00)')
" 2>/dev/null || warn "Could not parse report (may not be JSON — check file manually)"

echo ""
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
echo "  ✔ Done — $(date)"
echo "  Report: $REPORT_FILE"
echo "━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━━"
